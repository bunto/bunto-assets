...
module Bunto
  module RSpecHelpers
    ...
    # Stubs the asset config on Bunto for you, this is meant to be used
    ...
    def stub_asset_config(inst, hash = nil)
      ...
      hash = Bunto::Assets::Config.merge(hash)
    ...
    def stub_env_config(inst, hash = nil)
      ...
      hash = Bunto::Utils.deep_merge_hashes(inst.asset_config, hash)
    ...
    # Stubs the Bunto site with your configuration, most of the time you
    ...
    def stub_bunto_site(opts = {})
      path = File.expand_path("../../fixture", __FILE__)
      Bunto::RSpecHelpers.cleanup_trash

      silence_stdout do
        Bunto::Site.new(Bunto.configuration(opts.merge({
          "source" => path, "destination" => File.join(path, "_site")
        }))).tap(&:read)
      end
    end

    # See: `#stub_bunto_site` except this also kicks a process so that
    #   do both the building and the processing all in one shot.

    def stub_bunto_site_with_processing(oth_opts = {})
      site = stub_bunto_site(oth_opts)
    ...
    # through us Bunto created, so that if we test again there is nothing but
    # clean data to test with, and not dirty old data.

    def self.cleanup_trash
      %W(.asset-cache .bunto-metadata _site).each do |v|
...
Liquid::Template.register_tag "context_thief", \
  Bunto::RSpecHelpers::ContextThief


RSpec.configure do |c|
  c.include Bunto::RSpecHelpers
  c.before(:all) { Bunto::RSpecHelpers.cleanup_trash }
  c.after (:all) { Bunto::RSpecHelpers.cleanup_trash }
end
