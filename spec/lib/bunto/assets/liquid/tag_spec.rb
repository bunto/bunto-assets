...
describe Bunto::Assets::Liquid::Tag do
  ...
  it "allows the user to use Liquid variables" do
    expect(stub_tag(:img, "'ruby.png' version:'{{ bunto.version }}'"). \
      attr("version")).to(eq(Bunto::VERSION))
  end
  ...
  it "captures and outputs errors" do
    expect(Bunto.logger).to receive(:error).and_return nil
